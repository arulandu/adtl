...
let rec occurs x = function
  | Var y -> x = y
  | Universe _ | Nat | Zero -> false
  | Pi (y, e1, e2)
  | Lambda (y, e1, e2) -> occurs x e1 || (x <> y && occurs x e2)
  | App (e1, e2) -> occurs x e1 || occurs x e2
  | Succ s -> occurs x s
  | ElimNat (e1, e2, e3, e4) -> occurs x e1 || occurs x e2 || occurs x e3 || occurs x e4
...
let beautify =
  let rec beautify sbst = function
    ...
    | Nat -> Nat
    | Zero -> Zero
    | Succ s -> Succ (beautify sbst s)
    | ElimNat (e1, e2, e3, e4) -> ElimNat (beautify sbst e1, beautify sbst e2, beautify sbst e3, beautify sbst e4)
